# Zaposli.ba — Werkspot-Style Feature Completion
...
- Designer standard: "Collins" — senior web designer, 10 years, Pentagram-level detail, no em-dashes, compact spacing
...
1. **Client dashboard** (`/dashboard/`):
   ...
   - Buttons: "Otkaži posao", "Označi kao završen", "Pogledaj ponude"
2. **Job detail page** (`/dashboard/poslovi/[id]/`):
   ...
   - Client can "Prihvati ponudu" — sets bid status to accepted, job status to in_progress, rejects other bids
   - Client can "Označi kao završen" — sets job status to completed
   - Link to conversation with accepted firm
3. **Firm dashboard** (`/dashboard/firma/`):
   - Restore job list with "Pošalji ponudu" form
