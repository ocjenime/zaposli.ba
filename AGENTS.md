...
- Fixed visible seam between hero and the "Od ideje do gotovog posla" steps section on `/kako-funkcionise/`: hero now uses a single `bg-ink` base, the bottom fade is extended to `h-48 md:h-64` with `via-ink/80`, the steps section is solid `bg-ink` with a soft top glow, and a bottom gradient fade blends it into the light `Benefits` section. `npm run lint` and `npm run build` both pass (2405 pages).
