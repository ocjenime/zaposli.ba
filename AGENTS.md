...
- Replaced the reused `/images/poslovi-hero.jpg` on `/kako-funkcionise/` with a unique abstract animated Higgsfield background: dark gradient base, subtle mesh pattern, four slow-moving brand-orange/amber glass orbs (`animate-float-orb`, `animate-float-orb-slow`, `animate-float-orb-reverse`), and cinematic vignette overlays; removed the `next/image` import since no photograph is used. `npm run lint` and `npm run build` both pass (2405 pages).
