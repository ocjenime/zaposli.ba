...
- Added a premium mobile-only footer in `components/Footer.tsx` matching the provided mockup (desktop footer unchanged): brand + tagline + `BOLJI LJUDI. BOLJI PROJEKTI.`, 2-column directory (`Najtraženije kategorije` with 8 real categories + `Sve kategorije`, `Popularni gradovi` with 8 real cities + `Svi gradovi`), info links (`O nama`, `Kontakt`, `Uslovi korištenja`, `Privatnost`, `Centar pomoći`), static App Store / Google Play badges with an `Uskoro dostupno` pill (no dead links), inline SVG BiH flag + `Iz Bosne i Hercegovine za bolje majstore.`, script `Gradimo bolju sutra.`, email + copyright. Social icons omitted (no profiles configured yet).
  - `npm run lint` and `npm run build` pass (2409 pages).
