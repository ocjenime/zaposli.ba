...
- Renamed `/kako-radi/` to `/kako-funkcionise/`, updated Header, Footer, sitemap, and `/za-firme/` link; added permanent redirect from `/kako-radi/*` to `/kako-funkcionise/*` in `next.config.js`. Redesigned the page in Higgsfield style with a cinematic homepage-like hero using `/images/poslovi-hero.jpg`, cinematic overlays, animated glass orbs, fade-in animations, and glassmorphic role-selector cards integrated directly into the hero. Interactive role selector lets users choose "Tražim majstora" (client) or "Nudim usluge" (firm/majstor), with an inline segmented switcher and dynamic 3-step cards with role-specific CTAs. Removed the duplicate category-cards section. `npm run lint` and `npm run build` both pass (2405 pages).
