...
- Reverted `/za-firme/` to the previous longer Higgsfield layout after user feedback, then removed the "Kategorije poslova" project bento and "Prednosti platforme" benefits sections to declutter the page.
- Fixed button visibility on `/za-firme/`: `PricingCTA` non-popular buttons now use white text/white border on dark pricing cards; `AdPricingCTA` secondary buttons use transparent white text/border on dark ad cards so they remain readable in light mode.
- Grammar pass on `/za-firme/` and `FirmAdsTab`: fixed "Šta" → "Što", "u toku" → "tokom", "preko" → "putem", "donose nove poslove" → "vam donose nove poslove", replaced em/en dashes with ASCII hyphens, and improved ad-creation labels ("Preostalo uključenih mini oglasa", "zatraži uputstvo za uplatu").
