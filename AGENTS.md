...
- Fixed visible seam between hero and the "Od ideje do gotovog posla" steps section on `/kako-funkcionise/`: hero now uses `bg-gradient-to-b from-ink via-ink-900 to-ink-800`, the steps section continues the gradient with `bg-gradient-to-b from-ink-800 via-ink-950 to-cloud`, and a soft bottom glow melts the two sections together. A separate bottom fade blends the steps section into the light `Benefits` section. `npm run lint` and `npm run build` both pass (2405 pages).
