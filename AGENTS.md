...
- Added a premium mobile-only footer in `components/Footer.tsx` matching the provided mockup (desktop footer unchanged): brand + tagline + `BOLJI LJUDI. BOLJI PROJEKTI.`, 2-column directory with vertical dividers (`Najtraženije kategorije` with 8 real categories + `Sve kategorije`, `Popularni gradovi` with 8 real cities + `Svi gradovi`), info links (`O nama`, `Kontakt`, `Uslovi korištenja`, `Privatnost`, `Česta pitanja`), social icon boxes (Facebook linked to the real page; Instagram/YouTube/LinkedIn shown dimmed with `uskoro` until URLs arrive), static App Store / Google Play badges with an `Uskoro dostupno` pill (no dead links) side-by-side with the inline SVG BiH flag + `Iz Bosne i Hercegovine za bolje majstore.`, script `Gradimo bolje sutra.`, email + copyright in a bottom row.
  - `npm run lint` and `npm run build` pass (2409 pages).
