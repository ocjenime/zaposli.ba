...
- `/kategorije/` hero image uses `object-cover` with `object-[58%_center]` like the homepage hero so the woman tending the greenery (right of the main worker) stays visible on mobile.
- Lightened the homepage hero overlays twice (`from-ink-950/85 via-ink-950/60 to-ink-950/30` → `/60 /35 /10`, bottom `/70 /20 /30` → `/45 /10 /15`) so the background photo shows vivid color while text stays readable.
- Rebuilt `/kontakt/` exactly per the mobile mockup in premium Higgsfield style: cinematic hero (`kontakt-hero.png`, glass `KONTAKT` pill, `Tu smo da pomognemo.` headline), white form card overlapping the hero (`Tema poruke` select + name + email + message, topic prefixed into the saved message since the table has no subject column, same success/error states), 3 help cards that set the topic and scroll to the form, and a dark `Vaš feedback nam je važan.` banner with chat-bubble accent. Later compacted all sections (shorter hero, tighter card/fields, smaller headings) so the page fits with minimal scrolling like the mockup.
- Rebuilt `/za-firme/` as a full dark Higgsfield page exactly per the `1.png` mockup, using optimized `public/images/zafirme-hero.jpg` (from `2.png`, 1672px → 1600px JPG 175KB) as the hero background:
  - Hero with `ZA FIRME I MAJSTORE` eyebrow, `Vi radite posao. Mi vam dovodimo klijente.` headline, CSS glass notification cards (`Novi upit za vašu uslugu`, `Novi projekat`, `Upit za ponudu`) + `VI GRADITE BOLJU BIH` script, orange register CTA + glass `Pogledajte kako funkcioniše` play button, and the 3-check trust row.
  - 3 benefit cards (`Novi klijenti`, `Više povjerenja`, `Manje praznog hoda`), pricing `Jednostavni paketi za svaki biznis` with all 4 REAL plans (Besplatno/Start/Pro/Premium, launch prices, real feature lists, role-aware `PricingCTA`), kept `#reklame` ad cards + dark FAQ + `Spremni ste da rastete?` CTA card with chart graphic + trust footer row. The old light how-it-works timeline was dropped (covered by `/kako-funkcionise/`).
- Matched `/za-firme/` pricing exactly to the mockup: 3 horizontal cards (Besplatno/Start NAJPOPULARNIJI/Pro) with the short mockup feature lists, compact mobile sizing, role-aware buttons; removed the `Pogledajte kako funkcioniše` video button for now. Premium plan + launch/yearly pricing details still live on the dashboard pretplata page.
- Final `/za-firme/` pass per feedback: compacted hero spacing, restored the Premium plan with full real feature lists + launch badges + yearly note, pricing and ad cards in 2-column grids (2×2 packages, 2+1 ads).
- Fixed hero notification cards so all text is fully visible (service/city/time on one wrapping line, no truncation); restyled all 3 ad cards in the pricing-card language; pulled the benefit cards up with a negative-margin overlap so they are visible on first load.
- Ad cards now run 5 KM → 19 KM → 49 KM in a single horizontal 3-column row (via `order` classes).
- Converted `/za-firme/` pricing (4 plans) and ads (3 cards) to horizontal snap-scroll strips; the strip auto-starts on the `Najpopularniji` plan; listing ad button switched to orange `primary` like the others.
- Fixed admin missing new-job emails in `supabase/functions/notify-firms-on-job`: the function returned early with `No recipients` when zero firms matched the job category (e.g. stolarija), so the admin summary email never sent. Admin email now sends for EVERY new job (with firm count + fetch-error note if any). Push auto-redeploys the function via the GitHub workflow.
- Rebuilt `/gradovi/` exactly per the mobile mockup in premium style (user confirmed the design was for gradovi):
  - New `app/gradovi/GradoviClient.tsx`: cinematic hero (`gradovi-hero.jpg`, glass `GRADOVI` pill, `Pronađi majstora u svom gradu.` headline, white city search pill with orange button + live autocomplete dropdown that navigates to the city page or filters the list), `Popularni gradovi` 2-column cards (Sarajevo, Banja Luka, Mostar, Tuzla, Bihać, Velika Kladuša, Cazin, Zenica) with `Pogledajte sve` scroll link, `Svi gradovi` with search pill + Bosnian A-Z letter filter + alphabetical grouped rows, empty state.
  - `app/gradovi/page.tsx` is now a thin server wrapper keeping metadata; deleted obsolete `components/CityGrid.tsx` (green/red firm tints dropped with the old design).
  - Compacted the new `/gradovi/` hero + popular cards for mobile (smaller headline, tighter cards so long names like Velika Kladuša fit without truncation) and restyled `/gradovi/[slug]/` in the same premium language (cinematic `gradovi-hero.jpg` hero, light page bg, tighter sections; all data fetching unchanged).
- Redesigned `/blog/` exactly per the mobile mockup in premium style using all 8 articles:
  - New `app/blog/BlogClient.tsx`: cinematic hero (`kategorije-majstori.jpg`, orange `BLOG` eyebrow, `Bolji projekti. Stvarni savjeti.` headline, white search pill with orange arrow button, handwritten side note), horizontally scrollable category pills built from real article categories (`Svi članci`, `Cijene`, `Vodiči`, `Savjeti`), large featured photo card with glass category pill + arrow + orange dash + meta, 2-column photo grid, dark `Imate temu za članak?` CTA (`Pošalji prijedlog` → `/kontakt/`), `Najnoviji članci` horizontal scroll with reset link, and the `Iz Bosne i Hercegovine za bolje majstore.` quote.
  - `lib/articles.ts` gained a per-article `image` mapped to existing trade photos; live search + category filtering with counts and empty state; `app/blog/page.tsx` is now a thin server wrapper with metadata.
- Moved `RecommendedFirmsSection` below the job list on `/poslovi/` so the page starts with jobs.
- Rebuilt `/kategorije/` exactly per the new mobile mockup: cinematic hero (`herozaposli.png`, glass `KATEGORIJE` pill, `Pronađi majstora za svaki posao.` headline, white search bar that live-filters + scrolls), `Popularne kategorije` 2-column cards (8 real categories with their icons + `Pogledajte sve` scroll link), `Sve kategorije` with search pill + Bosnian A-Z letter filter + alphabetical rows, empty state. Restyled `/kategorije/[slug]/` heroes in the same compact cinematic language and tightened lower sections; all data, SEO and emergency flows unchanged.
- Added 2 missing trade categories (`masinsko-nabacivanje`, `tlakovi-estrih` with icons, prices, services, short names) plus `supabase/migration-new-categories-estrih-masinsko.sql` seed (user must apply in SQL Editor); build now generates 2502 pages.
...
- Rebuilt `/o-nama/` exactly per the mobile mockup in premium style: cinematic hero (`herozaposli.png`, visible breadcrumb, orange `O NAMA` eyebrow, `Svaki projekat` headline, orange CTA, handwritten side note), `Zasto smo napravili` split with photo card (`trazim-majstora-card.jpg`, `Od ideje do gotovog projekta.`), 4 mini value cards, `Nasa ideja` split (`kontakt-hero.png`, bold closing lines), `Tek smo poceli` gray card with handwritten note, dark CTA banner (`zafirme-hero.jpg`, `Tvoj projekat. Pravi majstori.`). Metadata + Organization schema kept; old timeline/bento sections replaced.
- Rebuilt `/top-firme/` exactly per the mobile mockup: cinematic hero (`POVJERENJE STVARA REZULTATE.` pill, `Top firme` headline, 3 trust items, white search pill with orange `Pretrazi` button), scrollable category pills from real data + `Vise` link, dark promo banner (`Samo najbolje za vas projekat.` + `Kako funkcionise?` link), 4 working sort tabs (Top/Najbolje ocijenjene/Najvise projekata via new secure `get_firm_project_counts` RPC/Nedavno aktivne), firm rows with logo, verified badge, rating, real project counts, city, category chips and arrow CTA, plus `Vasa firma moze biti ovdje` CTA. New `supabase/migration-firm-project-counts.sql` must be applied in SQL Editor (graceful fallback to 0 until then).
