...
- Restored `/za-firme/ZaFirmeContent.tsx` from commit `b3db6fae` (the last known-good version with correct Bosnian diacritics) and removed only the "Kategorije poslova" project showcase section using a Python script so the rest of the text remained untouched.
