...
export default function RoleCTACards() {
  return (
    <section className="relative py-2.5 md:py-6 bg-cloud px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-2.5 md:gap-6">
          {cards.map((card) => (
            <Link
              key={card.href}
              href={card.href}
              className="group relative flex items-center gap-3 md:gap-5 rounded-xl md:rounded-2xl bg-ink-900/60 backdrop-blur-sm border border-ink-800 hover:border-brand-orange/40 transition-all duration-300 p-3 md:p-6 overflow-hidden"
            >
              <div
                className={`shrink-0 w-9 h-9 md:w-16 md:h-16 rounded-lg md:rounded-2xl flex items-center justify-center transition-colors ${
                  card.theme === 'orange'
              ...
              <span className="shrink-0 w-7 h-7 md:w-10 md:h-10 rounded-full bg-ink-800 text-white/60 group-hover:bg-brand-orange group-hover:text-white transition-colors flex items-center justify-center">
