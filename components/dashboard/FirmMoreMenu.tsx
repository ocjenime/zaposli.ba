...
import { X, User, Building2, MessageSquare, Bell, Sun, LogOut, Settings } from 'lucide-react';
import { useAuth } from '@/lib/auth-context';
import ThemeToggle from '@/components/ThemeToggle';
import NotificationBell from '@/components/NotificationBell';

interface FirmMoreMenuProps {
  open: boolean;
  onClose: () => void;
}

export default function FirmMoreMenu({ open, onClose }: FirmMoreMenuProps) {
  const { user, role, isAdmin, signOut } = useAuth();

  const dashboardHref = role === 'firm' || role === 'majstor' ? '/dashboard/firma/' : '/dashboard/';
  const profileHref = role === 'firm' || role === 'majstor' ? '/dashboard/firma/profil/' : '/dashboard/profil/';
    ...
    <div className="md:hidden fixed inset-0 z-50">
      ...
      <div className="absolute left-0 right-0 bottom-0 bg-white dark:bg-ink-950 rounded-t-3xl shadow-2xl animate-slide-up max-h-[85vh] overflow-y-auto">
        ...
        <nav className="p-3 space-y-1">
          ...
          >
            <Building2 className="w-5 h-5 text-brand-orange" /> Dashboard
          </Link>
          <Link
            href={profileHref}
            onClick={onClose}
          ...
          <Link
            ...
            className="flex items-center gap-3 px-3 py-3 rounded-xl text-gray-700 dark:text-white/90 font-medium hover:bg-gray-50 dark:hover:bg-ink-800 transition-colors"
          >
            <Settings className="w-5 h-5 text-brand-orange" /> Pretplata
          </Link>
          <button
            type="button"
            onClick={() => {
