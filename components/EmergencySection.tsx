      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-2xl md:text-3xl font-extrabold text-[#ffffff] mb-2.5 tracking-tight">
              Kvar ne čeka. Majstor dolazi odmah.
            </h2>
            <p className="text-[#ffffff]/55 text-sm md:text-base mb-4 max-w-xl leading-relaxed">
            ...
            <div className="flex flex-wrap gap-1.5">
              {cat.services.map((s) => (
                <span key={s} className="px-2.5 py-1 bg-[#ffffff]/5 border border-[#ffffff]/10 rounded-md text-xs text-[#ffffff]/70">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div className="flex sm:flex-row lg:flex-col gap-3 shrink-0">
            <Link
              ...
              className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-red-600 to-red-700 text-[#ffffff] px-6 py-3.5 rounded-xl font-bold hover:shadow-xl hover:shadow-red-600/25 transition-all active:scale-95"
            >
              Objavi hitan posao
            ...
            <Link
              ...
              className="inline-flex items-center justify-center gap-1.5 text-[#ffffff]/60 hover:text-[#ffffff] text-sm font-medium transition-colors px-6 py-2"
            >
              Kako funkcioniše
        ...
        <div className="mt-7 pt-6 border-t border-[#ffffff]/10 flex flex-col sm:flex-row gap-4 sm:gap-2">
          {steps.map((step, i) => (
            <div key={step.title} className="flex items-start gap-3 sm:flex-1 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[#ffffff]/45 text-xs mt-0.5 leading-relaxed">{step.text}</div>
              </div>
              {i < steps.length - 1 && (
                <ChevronRight className="hidden sm:block w-4 h-4 text-[#ffffff]/15 shrink-0 self-center mx-2" />
              )}
            </div>
          ))}
        </div>
